## this is the object that manages all things related to the Prints addon
## 
...
## when designing new Prints types it is very important not to use opaque fields
...
## extends PrintsContact. what we don't want is a field that just accepts a JSON/Dictionary
## since those both would not have an enforced schema, thus making documentation the only 
## way to know what they do.
## [br][br]because of how the Prints addon is designed, it ensures that all pieces are predictable
